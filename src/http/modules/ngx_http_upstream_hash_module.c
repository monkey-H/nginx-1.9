    ...
    ngx_http_upstream_srv_conf_t *us)
{
    ...
    fprintf(stderr, "consistent hash init hash peer %ld \n", 1000000*start.tv_sec+start.tv_usec);
...
ngx_http_upstream_get_hash_peer(ngx_peer_connection_t *pc, void *data)
{
    ...
    fprintf(stderr, "consistent hash end %ld \n", 1000000*end.tv_sec+end.tv_usec);
    fprintf(stderr, "consistent hash last %ld \n", 1000000*(end.tv_sec-start.tv_sec)+end.tv_usec-start.tv_usec);
...
ngx_http_upstream_init_chash(ngx_conf_t *cf, ngx_http_upstream_srv_conf_t *us)
{
	...
	fprintf(stderr, "consistent hash server hash %u \n", points->point[i].hash);
	if (i==0) {
	    val1 = points->point[0].hash;
	    continue;
	}
	if (0 == ngx_strncmp(points->point[0].server->data, points->point[i].server->data, (int)points->point[0].server->len)) {
	    fprintf(stderr, "consistent hash server hash %d \n", 1);
	    fprintf(stderr, "consistent hash server name %*s \n", (int)points->point[i].server->len, points->point[i].server->data);
	    val1 = val1 + points->point[i].hash - points->point[i-1].hash;
	} else {
	    fprintf(stderr, "consistent hash server hash %d \n", 2);
	    fprintf(stderr, "consistent hash server name %*s \n", (int)points->point[i].server->len, points->point[i].server->data);
	    val2 = val2 + points->point[i].hash - points->point[i-1].hash;
	}
    }
    fprintf(stderr, "consistent hash server val1 %u \n", val1);
    fprintf(stderr, "consistent hash server val2 %u \n", val2);
